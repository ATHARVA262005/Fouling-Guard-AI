...
def predict_fouling(image_tensor):
    ...
    if model is None:
        ...
        coverage = max(20, min(95, int(random.gauss(75, 18))))
        
        high_risk_species = [1, 6, 8]
        if coverage > 80 or species_id in high_risk_species:
            criticality = 'High'
    ...
    try:
        with torch.no_grad():
            ...
            # Coverage prediction (sigmoid to get 0-100%)
            coverage = torch.sigmoid(coverage_raw).item() * 100
            coverage = max(5, min(95, int(coverage)))
            
            # Determine criticality based on coverage and species
            high_risk_species = [1, 6, 8]  # Balanus Amphitrite, Perna Viridis, Saccostrea
            if coverage > 75 or species_pred in high_risk_species:
                criticality = 'High'
            elif coverage > 40:
                criticality = 'Medium'
            else:
            ...
            print(f"🤖 REAL MODEL PREDICTION: {SPECIES_MAP.get(species_pred)} - {coverage}% - {criticality}")
            
            return {
                'species': SPECIES_MAP.get(species_pred, 'Unknown Species'),
                'coverage': coverage,
                'criticality': criticality,
...
def predict():
    try:
        ...
        # Make prediction
        prediction = predict_fouling(image_tensor)
        
        # Calculate additional metrics
        fuel_penalty = max(5, int(prediction['coverage'] * 0.3))
        ...
        # Generate response in client format
        response = {
            'success': True,
            'analysis': {
                ...
                'method': method,
                'urgency': urgency,
                'note': f"Biofouling analysis complete. {prediction['species']} detected with {prediction['coverage']}% coverage."
            },
            'timestamp': '2024-01-01T00:00:00Z'
        }
